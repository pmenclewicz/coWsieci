...
def generate_ai_image(image_prompt, output_filename):
    """Generuje zdjęcie nagłówkowe (Imagen 3) i mocno je kompresuje, aby oszczędzać miejsce."""
    try:
        ...
        for generated_image in result.generated_images:
            # Wczytujemy surowy obraz z bajtów do Pillow (PIL)
            img = Image.open(io.BytesIO(generated_image.image.image_bytes))
            
            # Skalujemy szerokość do 600px (zmniejsza wagę pliku do ~30-50 KB)
            img.thumbnail((600, 337))
            
            # Zapisujemy z kompresją JPG (quality=40)
...
def save_html_page(keyword, article_html, meta_desc, image_prompt):
    ...
    # Generowanie wykompresowanej grafiki AI
    success = generate_ai_image(image_prompt, image_filename)
    if not success:
        # Rezerwowy link, jeśli generowanie AI napotka limit
